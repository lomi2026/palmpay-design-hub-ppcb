...
- 当前生产仍为上一次已验收测试镜像的晋级版本；最新测试候选尚未晋级生产。
- 测试与生产均使用独立 PostgreSQL schema、钉钉统一登录和 `app_managed` 授权。
- PPCB 环境使用平台私有 OSS 文件能力；PPCB 运行不需要原 Cloudflare R2 凭据。

最新测试候选的两个实例全部就绪且无重启，启动均记录 `initialized:false`、保留现有组织，没有内容或封面导入日志。平台健康、内部健康检查、首页、未登录权限边界、真实登录后的 33 项项目库及 P01 新仓库外链已验收。当前生产镜像仍是旧启动行为；其上次启动日志确认执行过 48 条发布内容、49 个版本、17 个标签和 5 张封面的历史导入。
...
最新测试候选已取消普通启动中的内容/封面导入：已有组织不重新 seed，空环境仅一次性创建基础配置。没有新结构迁移时，只检查迁移账本，不重跑旧 SQL。该行为已发布到 testing，尚未晋级 production；当前生产旧镜像仍可能在重启时导入，不能作为无条件安全的回退版本。
...
- 新修改先在本地验收，再一次性发布测试。发现问题先保留证据、做只读诊断并向用户说明处理方案；阻断步骤在问题解决前不继续产生副作用，需要终止整次发布时先确认。不自动提交 PPCB 平台问题。
